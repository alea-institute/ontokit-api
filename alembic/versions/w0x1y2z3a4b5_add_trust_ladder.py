...
def upgrade() -> None:
    ...
    op.add_column(
        "suggestion_sessions",
        sa.Column("auto_accept_claimed_until", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "suggestion_sessions",
    ...
    op.create_index(
        "ix_suggestion_sessions_auto_accept_scan",
        "suggestion_sessions",
        ["auto_accept_after"],
        postgresql_where=sa.text(
            "status IN ('submitted', 'auto-submitted') "
            "AND auto_accept_after IS NOT NULL "
            "AND auto_accept_halted_at IS NULL "
            "AND NOT is_anonymous "
            "AND NOT is_llm_generated"
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("session_id", name="uq_suggestion_outcome_session"),
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index("ix_suggestion_sessions_auto_accept_scan", table_name="suggestion_sessions")
    op.drop_column("suggestion_sessions", "verification_passed")
    op.drop_column("suggestion_sessions", "auto_accept_claimed_until")
