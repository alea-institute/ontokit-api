...
async def list_member_trust(
    ...
    accepted_by_user = await service.count_accepted_by_user(
        project_id, [member.user_id for member in project.members]
    )
    ...
    for member in project.members:
        rows.append(
            MemberTrustResponse(
                ...
                accepted_count=accepted_by_user.get(member.user_id, 0),
            )
        )
    return rows
...
async def update_trust_settings(
    ...
    if (
        data.trust_promotion_threshold is not None
        and data.trust_promotion_threshold != project.trust_promotion_threshold
    ):
        project.trust_promotion_threshold = data.trust_promotion_threshold
        changed_fields.append("trust_promotion_threshold")
    if (
        data.auto_accept_quiet_days is not None
        and data.auto_accept_quiet_days != project.auto_accept_quiet_days
    ):
        project.auto_accept_quiet_days = data.auto_accept_quiet_days
        changed_fields.append("auto_accept_quiet_days")
    if data.auto_accept_enabled is not None and (
        ...
        changed_fields.append("auto_accept_enabled")

    if changed_fields:
        logger.info(
            "trust settings changed: project=%s actor=%s fields=%s",
            project_id,
            user.id,
            ",".join(changed_fields),
            extra={
                "event": "project_trust_settings_changed",
