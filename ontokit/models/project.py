...
class Project(Base):
    ...
    # --- Contribution trust ladder settings (R6, R11) ---
    # Accepted suggestions required before a signed-in contributor is
    # auto-promoted to trusted on this project (KTD8: default 5).
    trust_promotion_threshold: Mapped[int] = mapped_column(
        Integer, nullable=False, default=5, server_default="5"
    )
    # Auto-accept is OFF by default (KTD8) — a project owner opts in.
    auto_accept_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    # Quiet period, in days, before a trusted human suggestion auto-merges.
...
class ProjectMember(Base):
    ...
    # --- Contribution trust ladder (R4, R6; KTD1) ---
    # Materialized effective trust state. Auto-promotion may only flip this
    # while `trust_override` is "none".
    is_trusted: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    # Admin decision that outranks auto-promotion, and is sticky:
    #   none    — the ladder governs (auto-promotion may act)
    #   granted — admin granted trust directly
    #   refused — admin declined; the threshold must never auto-promote
    #   revoked — admin removed trust; the threshold must never re-promote
