...
class TrustLimitStatus(StrEnum):
    """Distinct limiter outcomes so callers do not conflate abuse with outage."""
...
class TrustLimitDecision:
    """Result of atomically consuming one untrusted submission slot."""

    status: TrustLimitStatus
    remaining: int

    @property
    def allowed(self) -> bool:
        return self.status is TrustLimitStatus.ALLOWED
...
async def check_and_consume(
    ...
    ``remaining`` is the count left AFTER this submission, or 0 on denial.
    Exhaustion and infrastructure unavailability are deliberately distinct so
    the HTTP layer can return 429 for the former and a retryable 503 for the
    latter.
    ...
    if daily_limit <= 0:
        return TrustLimitDecision(TrustLimitStatus.EXHAUSTED, 0)

    if redis is None:
        _alert_unavailable(project_id, user_id, RuntimeError("no redis client configured"))
        return TrustLimitDecision(TrustLimitStatus.UNAVAILABLE, 0)
    ...
    if used > daily_limit:
        return TrustLimitDecision(TrustLimitStatus.EXHAUSTED, 0)
    return TrustLimitDecision(TrustLimitStatus.ALLOWED, daily_limit - used)
