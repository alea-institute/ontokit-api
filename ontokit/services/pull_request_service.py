...
class PullRequestService:
    ...
    async def update_pull_request(
        ...
        # unified reconciler replays the complete title/body rather than only
        # the fields present in this HTTP request.
        await self.db.commit()
    ...
    async def _perform_github_sync(
        ...
        *,
        ...
        if not finished and repair_budget > 0:
            # This attempt performed remote I/O after a newer local intent was
            # claimed. Its side effect may have landed last, so actively replay
            # the newest durable row instead of merely discarding its receipt.
    ...
    async def _get_github_token(self, project_id: UUID) -> tuple[GitHubIntegration, str] | None:
        """Resolve the system mirror credential, with its transitional fallback."""
        integration = await self._get_github_integration(project_id)
        if not integration or not integration.sync_enabled:
            return None
