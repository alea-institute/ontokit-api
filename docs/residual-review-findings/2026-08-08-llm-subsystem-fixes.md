...
| P0-6 | `7b60405b`, `66ef1201` | `b6d002e7`: a real pgvector row with an identical label/vector and no parent returned `pass` at the structural 0.80 ceiling. | `test_p0_6_identical_real_embedding_blocks_without_structure` returns `block` with score 1.0 both without a parent and with an unindexed parent; unavailable structural weight is renormalized without lowering thresholds. |
...
| P1-2 | `698aa4ce`, `b43fc7ab`, `66ef1201` | At `88de4244`, provider embedding calls had no budget/audit path and active-job uniqueness was branch-scoped in code only. | Paid embedding operations check budget before invocation and write audit cost; the migration reconciles legacy duplicate active jobs before installing a project-wide partial unique index. |
...
| P1-8 | `35b97ef8`, `b6960cb3` | Process-local branch locks could not serialize four production workers. | Git writes take a Postgres transaction advisory lock keyed by project and branch; validation and session refresh occur after lock acquisition. |
...
- Full suite: `.venv/bin/pytest -q` — **2573 passed**, including 26 live
