...
async def test_project_lock_serializes_pr_numbers_for_disjoint_branches(
    ...
    async def allocate(source: str, target: str) -> int:
        async with (
            ...
            # Widen the max-plus-one race window: without the project lock,
            # both disjoint branch pairs would select the same value.
...
async def test_suggestion_submit_acquires_pr_lock_set_once(
    ...
    service._enqueue_branch_refresh = AsyncMock()  # type: ignore[method-assign]
