...
class TestCommitIdentityEndpoints:
    ...
    def test_patch_explicit_null_clears_email_verification_and_opt_in(
        self, authed_client: tuple[TestClient, AsyncMock]
    ) -> None:
        client, session = authed_client
        preference = MagicMock()
        ...
        response = client.patch("/api/v1/users/me/commit-identity", json={"commit_email": None})

        assert response.status_code == 200
        assert preference.commit_email is None
        assert preference.commit_email_verified is False
        assert preference.use_verified_email is False

    def test_patch_omitted_email_preserves_address(
        self, authed_client: tuple[TestClient, AsyncMock]
    ) -> None:
        client, session = authed_client
        preference = MagicMock()
        ...
        response = client.patch(
            "/api/v1/users/me/commit-identity", json={"use_verified_email": False}
        )

        assert response.status_code == 200
        assert preference.commit_email == "1234+t@users.noreply.github.com"
        assert preference.commit_email_verified is True
        assert preference.use_verified_email is False
