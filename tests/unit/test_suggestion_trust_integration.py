...
def _capability_result(project: MagicMock, member: MagicMock | None) -> MagicMock:
    result = MagicMock()
    result.one_or_none.return_value = (project, member)
    return result


def _outcome_counts_result(accepted: int, total: int) -> MagicMock:
    result = MagicMock()
    result.one.return_value = (accepted, total)
    return result
...
class TestMintingGate:
    def _save(self, mints: bool, content: str) -> SuggestionSaveRequest:
    ...
    async def test_anonymous_save_derives_minting_without_client_hint(
        ...
        mock_db.execute.side_effect = _results(
            _result_for(session), _result_for(project), project=project
        )
        service.git_service.get_file_from_branch = MagicMock(
            return_value=b"@prefix : <http://x#> ."
        )
...
class TestCapabilities:
    async def test_untrusted_sees_threshold_and_no_minting(
        ...
        mock_db.execute.side_effect = [
            _capability_result(project, project.members[0]),
            _outcome_counts_result(2, 3),
        ]
    ...
    async def test_signed_in_nonmember_cannot_probe_private_project(
        self, service: SuggestionService, mock_db: AsyncMock
    ) -> None:
        project = _project([])
        project.is_public = False
        ...
        with pytest.raises(HTTPException) as exc:
            await service.get_capabilities(PROJECT_ID, _user("stranger"))
        assert exc.value.status_code == 403
...
class TestUntrustedSubmissionLimiter:
    async def test_limiter_outage_is_retryable_503_not_quota_429(
...
class TestTriageQueue:
    ...
    async def test_linked_pull_requests_are_loaded_in_one_batch(
        self, service: SuggestionService, mock_db: AsyncMock
    ) -> None:
        project, sessions = self._fixture()
        pull_requests = []
        for index, session in enumerate(sessions, start=1):
            ...
            pull_request.id = session.pr_id
            pull_request.github_pr_url = f"https://example.test/pull/{index}"
            pull_requests.append(pull_request)
        pr_result = MagicMock()
        pr_result.scalars.return_value.all.return_value = pull_requests
        mock_db.execute.side_effect = [
            _result_for(project),
            _sessions_result(sessions),
            pr_result,
        ]

        result = await service.list_pending(PROJECT_ID, _user("reviewer-1"))

        assert [item.github_pr_url for item in result.items] == [
...
class TestBulkReview:
    async def test_bulk_dismiss_processes_every_id(
        ...
        for sid in ("s_1", "s_2", "s_3"):
            results.append(_result_for(sessions[sid]))
